			...
			for (j = 0; j < numcols; j++) {
				...
				if (maparray[i,j] == 0) {
					...
					if (thePrefab != ""){
						...
						GameObject clone = Instantiate(prefab, new Vector3(j + Xoff, numrows - i +Yoff, 0), Quaternion.Euler(0, 0, Zrot)) as GameObject;
						clone.transform.localScale = new Vector3(Xscale,Yscale,1); 
